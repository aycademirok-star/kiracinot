...
export default function AdminPage() {
  ...
  const [yorumlar, setYorumllar] = useState<any[]>([]);
  ...
  async function yorumuOnayla(id: number) {
    await supabase.from("reviews").update({ onaylandi: true }).eq("id", id);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-zinc-500">
            {bekleyenYorumlar.length} bekleyen yorum
          </span>
        </div>
      </div>
